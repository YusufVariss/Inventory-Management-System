...
const StockIn = () => {
  const [stockInTransactions, setStockInTransactions] = useState([
    {
      id: 1,
    ...
    },
    {
      ...
      date: '2024-01-14',
      status: 'pending',
      notes: 'Tedarikçi onayı bekleniyor'
    },
    {
      id: 3,
  ...
  const suppliers = ['Tech Accessories Ltd.', 'Power Solutions Co.', 'Office Supplies Inc.', 'Digital World Ltd.'];
  ...
  const handleAddStockIn = () => {
    if (newStockIn.productName && newStockIn.quantity && newStockIn.unitPrice && newStockIn.supplier) {
      const transaction = {
        id: Date.now(),
        ...newStockIn,
        quantity: parseInt(newStockIn.quantity),
        unitPrice: parseFloat(newStockIn.unitPrice),
        totalPrice: parseInt(newStockIn.quantity) * parseFloat(newStockIn.unitPrice),
        date: new Date().toISOString().split('T')[0],
        status: 'pending'
      };
      setStockInTransactions([...stockInTransactions, transaction]);
      setNewStockIn({
        productName: '',
        category: '',
        quantity: '',
  ...
  const updateTransactionStatus = (id, newStatus) => {
    setStockInTransactions(stockInTransactions.map(transaction => 
      transaction.id === id ? { ...transaction, status: newStatus } : transaction
    ));
  };

  const getTotalValue = () => {
    return stockInTransactions.reduce((total, transaction) => total + transaction.totalPrice, 0);
  };

  const getTransactionsByStatus = (status) => {
    return stockInTransactions.filter(transaction => transaction.status === status).length;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Stok Girişi</h1>
          <p className="text-gray-600 dark:text-gray-400">Yeni ürün ekleyin ve mevcut stokları artırın</p>
        </div>
        <button
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stockInTransactions.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      {showAddStockIn && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Yeni Stok Girişi</h3>
              <div className="space-y-4">
                <input
                  type="text"
                  placeholder="Ürün Adı"
                  value={newStockIn.productName}
                  onChange={(e) => setNewStockIn({...newStockIn, productName: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                />
                <select
                  value={newStockIn.category}
                  onChange={(e) => setNewStockIn({...newStockIn, category: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                >
                ...
                <input
                  ...
                  value={newStockIn.unitPrice}
                  onChange={(e) => setNewStockIn({...newStockIn, unitPrice: e.target.value})}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                />
                <select
